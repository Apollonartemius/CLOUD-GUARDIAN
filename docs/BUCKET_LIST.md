...
| 11 | CI security scanning (trivy / pip-audit / Dependabot) | OPEN | Medium | Small | Dependencies are not auto-vulnerability-checked |
...
| One-command "full self-healing" demo script (chaos → incident → remediation → resolve, captured for the report) | OPEN | `scripts/demo.sh` exists but doesn't prove the full resolve cycle on exit |
| OIDC SSO live end-to-end test | OPEN | Blocked: needs the user's real Google OAuth client + `OIDC_ENABLED=true` |
| Run the full test suite on Python 3.11 (CI path) | OPEN | Host is Python 3.14 → 7 `google.protobuf`-related test failures; suite is green on 3.11/CI |
...
| 7 host-side test failures | Python 3.14 vs `google.protobuf`; green on Python 3.11 / CI |